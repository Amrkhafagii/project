/*
  # Fix Performance Optimization with Materialized Views
  ...
  2. Indexes
    ...
    - Partial indexes for filtered queries (without date functions)
*/
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_weekly_summary_user_week ON weekly_summary_cache(user_id, week_start);
CREATE UNIQUE INDEX IF NOT EXISTS idx_monthly_trends_user_month ON monthly_trends(user_id, month);
CREATE UNIQUE INDEX IF NOT EXISTS idx_user_stats_user ON user_stats_cache(user_id);
...
-- Create partial indexes for filtered queries (without date-based conditions)
CREATE INDEX IF NOT EXISTS idx_active_challenges ON social_fitness_challenges(start_date, end_date) 
WHERE is_public = true;

-- Create regular index for recent workouts instead of partial index
...
-- Create a scheduled job function to refresh views periodically
...
BEGIN
  ...
  -- For now, it just calls the refresh function
  PERFORM refresh_fitness_materialized_views();
END;
$$ LANGUAGE plpgsql;
